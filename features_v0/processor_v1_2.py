...
def process_cell_features(max_suffix=171, cell_ids=None, sat_ids=None, mode=1):
    """
            ...
            if mode == 1: # Process using cell_ids (cell mode)
                ...
                for sat in tqdm(sat_ids, desc=f"Processing satellites for suffix {sufix}", leave=False):
                    ...
                    if signals_sat:
                        sat_count = 0
                        error_count = 0
                        for idx, (cell, signal) in enumerate(signals_sat):
                            try:
...
if __name__ == "__main__":
    ...
    process_cell_features(max_suffix=max_suffix, sat_ids=sat_ids, mode=mode)
    
    # For cell mode (mode=1), provide cell_ids:
    # max_suffix = 171
    # cell_ids = list(range(39, 40))
    # mode = 1
    # process_cell_features(max_suffix=max_suffix, cell_ids=cell_ids, mode=mode)
